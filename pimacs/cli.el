...
;;; pimacs/pimacs/cli.el --- See README.md -*- lexical-binding: t; -*-
...
;; Generate all keys refcard cli generation for modified keymap bindings.
...
(defcli! (keys generate-all-refcard) () ;; TODO : add option to generate all the binding in one file.
           ...
           (require 'doom-start)
            (dolist (km pim-keymapname-alist)
              ...
              (setq
               keymapname (car km)
               module (cdr km))
              (print! (format "Generating %s key binding from module %s..." keymapname module))
              (pim-keys-bindings-to-md-refcard keymapname module)
              ))))

(provide 'pimacs/pimacs/cli)
;;; cli.el ends here
