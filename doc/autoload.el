...
(defun pim-which-key-insert-bindings-recursively-org (prefix level &optional subkeys)
  ...
  (let* ((which-key-max-description-length 1000)
    ...
    (dolist (key keys)
      (let* ((prefixn (s-trim (format "%s %s" prefix (pim-keystring-kbd-consolidate (pop key)))))
        ...
        (when isPrefix (pim-which-key-insert-bindings-recursively-org prefixn (+ 1 level) sub-keys))
        )
      )
    )
  )
