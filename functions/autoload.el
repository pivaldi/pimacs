...
;;;###autoload
(defun pim/keys-bindings-to-md-refcard (keymapnames fname)
  ...
  (let* ((which-key-max-description-length 1000)
    ...
    (unless keymapnames
      ...
      (dolist (km pim-keymapname-alist)
        (setq keymapnames (append keymapnames (cdr km)))
        ))
    (with-temp-file fname
      (insert (format "# PIMacs bindings%s\n\n" keyboard))
      (insert "**DO NOT EDIT THIS FILE**, it is auto-generated by the PIMacs cli.\n")
      (dolist (keymapname keymapnames)
        (unless (stringp keymapname) (error (format "keymap name `%s' is not a string." keymapname)))
        (setq keymap (symbol-value (intern keymapname)))
        (unless (keymapp keymap) (error (format "keymap %s is not a valid keymap." keymapname)))
        (insert (format "\n## Keymap %s\n\n" keymapname))
