...
If KEYMAPNAMES is nil, use the variable `pim-keymapname-alist' instead."
  ...
  (let* (
         ...
         (byte-compile-current-file t) ;; Needed by which-key--get-bindings
         (line nil)
         (pim-regexp "\\(.*\\)#pim *$")
         (keymapname nil)
         (keyboard (if (modulep! :pimacs keys +azerty) " (with option +azerty)" ""))
         (keys nil)
         (_ (require 'doom-start)))
    (unless keymapnames
      (setq keymapnames '())
      (dolist (km pim-keymapname-alist)
        (push (car km) keymapnames)
        ))
    (with-temp-file fname
      ...
      (dolist (keymapname keymapnames)
        ...
        (setq keys (which-key--get-bindings nil (symbol-value (intern keymapname)) nil t))
        (dolist (key keys)
