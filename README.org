...
#+html: <p align="center"><img src="./assets/pimacs-1280.webp" alt="Logo pimacs" width="1280" /></p>
...
programming oriented with special attention to French keyboard and automation of
installation of needed external tools handled by [[https://mise.jdx.dev/][MISE-EN-PLACE]].

- I use daily this configuration for professional programming, [[https://blog.piprime.fr/][bloging]] and Linux administration system works.\\
...
- [[https://www.gnu.org/software/emacs/manual/html_node/efaq/New-in-Emacs-29.html][Emacs 29+]] compiled with  [[https://tree-sitter.github.io/tree-sitter/][Tree-sitter]] support.

*** Optional  dependency
In order to support automation of installation of the needed external tools (LSP, Linters, Deboger etc), you need to [[https://mise.jdx.dev/getting-started.html][install Mise]] and enable the [[file:mise/README.org][Pimacs module Mise]].
...
- You can enable PIMacs in two ways :
  1. *Full featured PIMacs as it satisfy MY daily coding needs*\\
     ...
     This will load /all the PIMacs modules/ at once. See the file [[init.el]].
...
** Docker Testing Environment
   :PROPERTIES:
   :CUSTOM_ID: docker-testing-environment
   :END:
...
The Docker image includes Ubuntu 24.04, Emacs 30, Doom Emacs, and PIMacs modules.
See [[file:docker/README.org][docker/README.org]] for detailed documentation, usage examples, and troubleshooting.
...
- *pimacs/default*: [[file:default/README.org][PIMacs default configuration]].
- *pimacs/dired*: [[file:dired/README.org][Dired mode specific configuration]]. See also the auto-generated
  [[file:dired/dired-key-bindings-refcard.org][dired-mode-map key bindings refcard]] — split into a *PIMacs Added* section and a *Default Mode* section.
...
- *pimacs/functions*: [[file:functions/README.org][Define useful functions used under the hood by PIMacs]].
- *pimacs/keys*: [[file:keys/README.org][PIMacs global-map configuration]]. See also the auto-generated
  [[file:keys/keys-key-bindings-refcard.org][global-map key bindings refcard]] — split into a *PIMacs Added* section and a *Default Mode* section.
...
- *pimacs/lang-protobuf*: [[file:lang-protobuf/README.org][Configuration for Protocol Buffers]].\\
  Support ~protobuf-ts-mode~ with full [[https://buf.build/][Buf]] toolchain integration
  (lint, format, build, breaking, generate) and Buf LSP.
...
- *pimacs/session*: [[file:session/README.org][Workspace/Session Persistence and Auto Restore Last Session]].
- *pimacs/theme*: [[file:theme/README.org][Slight customization of the Emacs theme depending the module options used]].
- *pimacs/tramp*: [[file:tramp/README.org][Slight configuration of TRAMP]].\\
...
- *pimacs/treesit*: Configure and use *the native tree-sitter major modes in Emacs 29+* and install a batch of grammars.
- *pimacs/treemacs*: slightly adds to the default [[https://docs.doomemacs.org/v21.12/modules/ui/treemacs/][Doom Treemacs package]].
...
- *pimacs/visual-fill-column*: [[file:visual-fill-column/README.org][Soft-wraps text at a specified column width]]
  without modifying file contents. Supports centering text and integrates
  with ~visual-line-mode~ and ~visual-wrap-prefix-mode~.
